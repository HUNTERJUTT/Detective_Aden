...
/* ---------------- DATA REGISTRY MODULES (REFRESHED LISTS) ---------------- */
const names = [
...
/* Balanced Medium Difficulty Riddle Database Matrix */
const balancedRiddles = [
    {
        clue: "The thief accidently left behind a trace of green paint on the lock.",
        criminalMatch: "painting some outdoor emergency exit signs with a can of bright green paint",
        strongFake: "cleaning off an old whiteboard using a strong blue color marker eraser",
        neutralFake: "sorting through wooden boxes without using any tools or chemicals"
    },
    {
        clue: "Security saw the suspect drinking a hot beverage out of a blue mug right before the alarm.",
        criminalMatch: "brewing some warm black tea inside my favorite custom blue ceramic mug",
        strongFake: "refilling my water bottle from the dispenser using a clear glass tumbler cup",
        neutralFake: "checking locker keys on the table while chewing on some mint gum"
    },
    {
        clue: "A torn scrap piece of a yellow sticky note was discovered crumbled under the desk.",
        criminalMatch: "scribbling a few quick reminders down on my pad of neon yellow sticky notes",
        strongFake: "reading through a long training handbook bound in a bright orange plastic cover",
        neutralFake: "sweeping dust out from underneath the furniture items near the entryway"
    },
    {
        clue: "The floor near the stolen item was covered in wet, muddy boot prints.",
        criminalMatch: "coming inside from checking the garden patches out in the heavy pouring rain",
        strongFake: "washing the indoor bathroom floors using clean soapy water and a white hand towel",
        neutralFake: "working at my desk all evening inside a warm, perfectly dry office layout"
    },
    {
        clue: "The culprit dropped a small metallic paperclip near the broken security wire.",
        criminalMatch: "organizing stacks of loose invoice printouts with a box of silver paperclips",
        strongFake: "sewing up a small tear on my uniform jacket using a thin needle and thread",
        neutralFake: "wiping down the computer keyboard keys with an electronic microfiber cloth"
    }
];

const innocentActivitiesPool = [
    "counting storage boxes inside the back supply room",
    "checking safety labels on the extra wall fire extinguishers",
    "tightening a loose hinge screw on the storage cabinet door",
    "sorting backup document files in alphabetical order",
    "rearranging empty chairs in the staff breakroom"
];
...
function buildFreshCaseMatrix(currentLevel) {
    ...
    // Dynamic, diverse speech patterns to avoid identical alibi phrasings
    ...
    suspectRoles.forEach((role, index) => {
        ...
        // Pick individual unique innocent tasks for this current lineup pool
        ...
        switch(role) {
            ...
            case "Innocent3":
                ...
                break;
            case "RedHerring":
                // Shares a similar context category but lacks the exact keyword match
    ...
    let briefDescriptionHTML = `🚨 <b>INCIDENT RADAR REPORT:</b> A high-profile <b>${selectedCrime}</b> was carried out inside the <b>${selectedPlace}</b>!<br><br>` +
                               `🔍 <b>INVESTIGATION LOG NOTE:</b> ${selectedRiddle.clue}<br><br>` +
                               `<i>Note: Look closely at details. Don't guess—match the clue precisely to the right statement!</i>`;
...
// Populate continuous background glowing environmental ambient particles
for (let i = 0; i < 40; i++) {
    ambientEbers.push({
        x: Math.random() * width,
...
function animateWeatherEngine() {
    ...
    // Render Ambient Particles (Ember Glow Traces)
    ...
    // Populate rain stream particles
